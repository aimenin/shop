...
export function* getSnapshoFromUserAuth(userAuth, additionalData) {
    try {
...
export function* signInWithGoogle() {
    try {
        ...
        yield getSnapshoFromUserAuth(user)
    } catch (error) {
        yield put(SignInFailure(error));
    }
}
...
export function* isUserAuthenticated() {
    try {
        ...
        if (!userAuth) return;
        yield getSnapshoFromUserAuth(userAuth);
    } catch (error) {
        yield put(SignInFailure(error));
    }
}
...
export function* signInAfterSignUp({payload: { user, additionalData}}) {
    yield getSnapshoFromUserAuth(user, additionalData);
}
